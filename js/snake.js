class Snake{
    constructor(snakeSpeed, snake_length,snakeBody, px,py, move_x, move_y,chg_x, chg_y){
    ...
    snake_direction(ev){     // Nemohu použít this - odkazuje se na event
        var key = ev.keyCode;
        switch(key){
            case 87  :
                if(snake.move_y <= 0){
    ...
    calc_tail(){
        ...
        this.snakeBody = this.snakeBody.slice(-(this.snake_length/(this.snakeSpeed)));
    }
    check_death(){
    ...
    checkAppleOnSnake(){
        if(this.snakeBody.some((snakePart)=> snakePart.x == apple.ax && snakePart.y == apple.ay))
